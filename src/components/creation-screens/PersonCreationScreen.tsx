...
const personSchema = z.object({
    ...
    type: z.enum(["animal", "human", "other"]).default("human"),
});
...
export default function PersonCreationScreen() {
    ...
    const form = useForm({
        ...
        onSubmit: async ({ value }) => {
            try {
                const personId = await editPerson(value);
                console.log("Personne créé:", personId);
        ...
        <div className="py-17.5 bg-bg min-h-screen">
            ...
            <div className="space-y-2.5 pb-32">
                <CreationSection label="Général">
                    <TextInput form={form} name="firstname" placeholder="Prénom" icon="title" />
                    <TextInput form={form} name="lastname" placeholder="Nom de famille" icon="title" />

                    <div className="grid grid-cols-2 gap-2">
                        <SelectInput form={form} name="type" placeholder="Type" icon={TbUser} options={[
                            { value: 'human', label: 'Humain' },
                            { value: 'animal', label: 'Animal' },
                        ]} />
                        <SelectInput form={form} name="gender" placeholder="Genre" icon={TbUser} options={[
                ...
                <CreationSection label="Lien">
                    ...
                    <SelectInput form={form} name="relation_type" placeholder="Type de relation" icon={TbUser} options={[
                    ...
                    <TextInput form={form} name="relation_name" placeholder="Lien précis. Ex : Cousin" icon="text" />
                    <TextArea form={form} name="description" placeholder="Pourquoi cette personne est-elle importante ?" />
                </CreationSection>
